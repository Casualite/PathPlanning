...
def occlusion(missing_triangles,arr_missing_centroids,normals,distance_d,scene,occlusion_checks=30,fov=60,width=640,height=480):
    ...
    for missing_tri_centroid,normal in zip(arr_missing_centroids,normals):            
            ...
            #print("1",occlusion_detected)
            # pcd1=np.vstack((viewpoint,missing_tri_centroid))
            # normal1=np.vstack((normal,-normal))
...
def not_occluded(scene,triangle_centre,viewpoint,up,fov,width,height):
    ...
    return new_tri_seen
# CHECK something is wrong the normals are always occluded for some reason i think mismatch between viewpoit,normal and where it is looking
# heirarchial clustering, want distance to be based on fov tan(fov)*distance_d
def clustering1(missing_tri,mesh,clusters=10,d_treshold=7):
    ...
    # print(triangle_centroids.dtype)
    # clustering = AgglomerativeClustering(n_clusters=None, memory='./caching_clustering',metric="euclidean",linkage="complete",compute_full_tree=True,distance_threshold=2*d_treshold)
    # print(clustering)
    # clustering.fit(triangle_centroids)
...
def after_occlusion_detection(mesh,cameras,scene,distance_d,clusters=10,iterations=100,fov=60,width=640,height=480):
    ...
    for i in range(iterations):
        ...
        new_points,view_normals,new_tri_seen,gotten=occlusion(missing_tri,clustered_triangle_centers,normals,distance_d,scene,fov,width,height)
        if(gotten):
...
def get_children_nodes(file_path="./meshes/B747_shifted_origin.ply",num_points=1000,distance_d = 7,fov=60,width=640,height=480,clusters=10,cluster_iterations=100,raycast=False):
    ...
    # mesh=remove_hidden_triangles(mesh,scene)
